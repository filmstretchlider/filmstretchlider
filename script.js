(function(){
  ...
  document.querySelectorAll('.js-open-whatsapp').forEach(bindTrigger);

  document.querySelectorAll('a[href*="wa.me/"]').forEach(link=>{
    if(!link.classList.contains('whatsapp-option')) bindTrigger(link);
  });

  document.querySelectorAll('[data-close-modal]').forEach(el=>{
    el.addEventListener('click',()=>modal&&modal.classList.remove('is-open'));
  });

  document.addEventListener('keydown',ev=>{
    if(ev.key==='Escape'&&modal) modal.classList.remove('is-open');
  });
})();